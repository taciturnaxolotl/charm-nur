...
func main() {
	...
	output := flag.String("output", "", "Output file (defaults to modules/crush/options.nix)")
	flag.Parse()
	...
	if err := tmpl.ExecuteTemplate(w, "root", map[string]interface{}{
		"Options": options,
	}); err != nil {
...
func nixDefault(val interface{}) string {
	if val == nil {
		return "null"
	}
	switch v := val.(type) {
